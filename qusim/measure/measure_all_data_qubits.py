...
def measure_all_data_qubits(rhoP, eta = np.array([[1,0,0],[0,0,0]])):
    ...
    for i in range(n, 0, -1):
        ...
        if p_out <= eta[mi, 0]:
            ...
            rhoP = (Proj1_gate_for_all_data_qubits_func[i-1] * rhoP) / P1
        M_data[i - 1] = m
        '''#if (random() <= (P0*(eta[0,0,0]+eta[0,0,1]) + P1*(eta[1,0,0]+eta[1,0,1]))):
        if random() <= P0:
            M_data[i - 1] = 1
            if P0 == 0:
                print('measure_all_data_qubits P0')
            #rhoP = (Proj0_gate(n = i, p = 1, cur = i - 1) * rhoP) / P0
            rhoP = (Proj0_gate_for_all_data_qubits_func[i-1] * rhoP) / P0
        else:
            M_data[i - 1] = -1
            if P1 == 0:
                print('measure_all_data_qubits P1')
            #rhoP = (Proj1_gate(n = i, p = 1, cur = i - 1) * rhoP) / P1
            rhoP = (Proj1_gate_for_all_data_qubits_func[i-1] * rhoP) / P1
        if M_data[i - 1] == 1:    
            if random() <= etaRO_0:
                M_data[i - 1] *= -1
