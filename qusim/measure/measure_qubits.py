...
def measure_last_num_qubits(rhoP, num, eta = np.array([[1,0,0],[0,0,0]])):
    '''
    Consistently measures all data qubit
    ...
    n = int(np.log2(rhoP.shape[0])/2)
    if (num > n):
        print('measure_last_num_qubits, num > n!')
    M_data = np.zeros((n-num))
    for i in range(n, n-num, -1):
        P0 = (2**(i)*rhoP[3, 0] + 1)/2
        P1 = 1 - P0
        if random() <= P0:
            mi = 0
